...
async def replay_unprocessed_ebay_fixed_price_transactions(
    *, limit: int = 200, event_id: str | None = None
) -> dict[str, Any]:
    """Re-process stored eBay FixedPriceTransaction notifications that were received but never
    ingested into the inventory pipeline (status='received')."""
    max_items = max(1, min(int(limit), 1000))
    query: dict[str, Any] = {
        "event_type": "GetItemTransactions",
        "status": "received",
    }
    if event_id:
        query["_id"] = str(event_id)

    cursor = db.ebay_notification_events.find(query).sort("received_at", 1).limit(max_items)
    events = [doc async for doc in cursor]

    if not events:
        return {"matched": 0, "processed": 0, "created_events": 0, "duplicates": 0, "errors": 0, "events": []}
    ...
    for event in events:
        ...
        # Extract order info already normalized by _parse_ebay_xml_notification at ingest time
        order = payload.get("order") or {}
        line_items = order.get("lineItems") or []
        if not line_items:
            # For records stored before the fix: Item and TransactionArray are already
            # parsed into the payload dict at the top level — extract directly.
            item = payload.get("Item") or {}
            tx_array = payload.get("TransactionArray") or {}
            tx = tx_array.get("Transaction") or {}
            if isinstance(tx, list):
                tx = tx[0]
            item_id = item.get("ItemID") or ""
            sku = item.get("SKU") or item.get("ApplicationData") or item_id
            ...
            if sku:
                line_items = [{"sku": sku, "quantity": qty, "itemId": item_id}]
                order = {"orderId": order_id, "lineItems": line_items}

        if not line_items:
            errors += 1
            out.append({"doc_id": str(doc_id), "status": "no_line_items"})
            await db.ebay_notification_events.update_one(
                {"_id": doc_id},
                {"$set": {"status": "replay_failed", "replay_error": "no_line_items", "replayed_at": _utc_now()}},
            )
            continue
        ...
        for li in line_items:
            sku = li.get("sku") or li.get("itemId") or ""
            if not sku:
                continue
            qty = max(1, _safe_int(li.get("quantity"), 1))
            order_id = order.get("orderId") or sku
            explicit_event_id = f"ebay-order:{order_id}:{sku}"
            try:
                result = await ingest_sale_event(
                    source_channel="ebay",
                    payload={"order_id": order_id, "line_item": li},
                    quantity_sold=qty,
                ...
                if status == "duplicate":
                    ...
                    created_events += 1
                event_results.append({"sku": sku, "status": status})
            except Exception as exc:
                errors += 1
                logger.exception("replay_ebay_fpt: ingest failed for sku=%s", sku)
                event_results.append({"sku": sku, "status": "error", "error": str(exc)})

        await db.ebay_notification_events.update_one(
            {"_id": doc_id},
            {"$set": {"status": "replayed", "replayed_at": _utc_now()}},
        )
        out.append({"doc_id": str(doc_id), "line_items": event_results})
